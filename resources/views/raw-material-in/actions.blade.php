@canany(['raw-material-in.edit', 'raw-material-in.delete'])
    <div class="dropdown">
        <button class="btn btn-sm btn-info dropdown-toggle" type="button" data-toggle="dropdown" aria-expanded="false">
            Aksi
        </button>
        <ul class="dropdown-menu">
            @can('raw-material-in.edit')
                <li>
                    <a href="{{ route('raw-material-in.edit', $rawMaterialIn->id) }}" class="dropdown-item">Edit</a>
                </li>
            @endcan
            @can('raw-material-in.delete')
                <li>
                    <form action="{{ route('raw-material-in.destroy', $rawMaterialIn->id) }}" method="post" class="d-inline">
                        @csrf
                        @method('delete')
                        <button type="submit" class="dropdown-item">Hapus</button>
                    </form>
                </li>
            @endcan
        </ul>
    </div>
@endcanany
